...
<body>
	<div class="top">>>数据维护</div>
	<div class="cx">
		操作日期：从 <input id="start" class="Wdate"
			onclick="WdatePicker({isShowWeek:true})" /> 到 <input id="end"
			class="Wdate" onclick="WdatePicker({isShowWeek:true})" />
			<button  onclick="shusel(1)">按时间查询</button>
		<p>
			操作人：<input id="name" />	<button  onclick="shusel(2)">按操作人查询</button>
		<p>
		操作内容：<input id="content" />	<button  onclick="shusel(3)">按内容查询</button>
	</div>
